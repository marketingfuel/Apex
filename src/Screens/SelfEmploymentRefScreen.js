...
export default function SelfEmploymentRefScreen({ navigation }) {
    ...
    saveReferences = async () => {
        ...
        try {
            ...
            let url = `self_reference?email=${auth?.email}&reference_type=${1}&p1_name=${firstName1}&p1_ocuppation=${Ocuppation1}&p1_address=${address}&p1_postcode=${postalCode}&p1_phone=${phoneNumber1}&p1_email=${auth?.email}&p1_known=${cardnumber1}&p2_name=${firstName2}&p2_ocuppation=${Ocuppation2}&p2_address=${address2}&p2_postcode=${postalCode2}&p2_phone=${phoneNumber2}&p2_email=${auth?.email}&p2_known=${cardnumber2}&self_employement=${referance ? 1 : 0}`;
            const result = await services.post1(url,auth?.token);
            console.log(result);
            navigation.navigate('CriminalOffensesScreen');
        }
        catch (ex) {
          console.log(ex);
        }
    ...
    getReference = async () => {
        ...
        try {
            const auth = await services.getAuthData();
            let url = `get_self_reference?email=${auth?.email}&reference_type=${1}`
